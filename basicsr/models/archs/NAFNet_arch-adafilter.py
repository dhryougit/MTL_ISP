...
class Adaptive_freqfilter_classification(nn.Module):
    def __init__(self):
        ...
        self.fclayer1 = nn.Linear(256, 10)
        self.fclayer2 = nn.Linear(256, 10)
    ...
    def forward(self, x):
        ...
        fq_mask = mask[0] + mask[1] + mask[2] + mask[3] + mask[4] + mask[5]+ mask[6]+ mask[7] + mask[8] + mask[9]
        
        fq_mask_max = torch.max(fq_mask)
        fq_mask_norm = fq_mask / torch.max(fq_mask)


        # plt.cla()
        # plt.clf()
        # plt.imshow(fq_mask_norm.detach().cpu().unsqueeze(0).permute(1,2,0))
        # plt.title("fq_mask"), plt.axis('off')
        # plt.savefig(f'fq_mask')


        lowpass = (x*fq_mask_norm)
