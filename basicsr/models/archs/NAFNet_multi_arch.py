...
class NAFNet_multi(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=net_bias)
        self.ending_noise = nn.Conv2d(in_channels=width, out_channels=img_channel, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=net_bias)
        self.ending_blur = nn.Conv2d(in_channels=width, out_channels=img_channel, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=net_bias)

        self.encoders = nn.ModuleList()
        self.decoders_noise = nn.ModuleList()
        self.decoders_blur = nn.ModuleList()
        self.middle_blks = nn.ModuleList()
        self.ups_noise = nn.ModuleList()
        self.ups_blur = nn.ModuleList()
        ...
        for num in dec_blk_nums:
            self.ups_noise.append(
                nn.Sequential(
                    nn.Conv2d(chan, chan * 2, 1, bias=False),
                    nn.PixelShuffle(2)
                )
            )
            self.ups_blur.append(
                nn.Sequential(
                    nn.Conv2d(chan, chan * 2, 1, bias=False),
                    nn.PixelShuffle(2)
                )
            ...
            chan = chan // 2
            self.decoders_noise.append(
                nn.Sequential(
                    *[NAFBlock(chan) for _ in range(num)]
                )
            )
            self.decoders_blur.append(
                nn.Sequential(
                    *[NAFBlock(chan) for _ in range(num)]
                )
            )
    ...
    def forward(self, inp, mode):
        ...
        if mode == 'noise':
            for decoder, up, enc_skip in zip(self.decoders_noise, self.ups_noise, encs[::-1]):
                x = up(x)
                x = x + enc_skip
                x = decoder(x)

            x = self.ending_noise(x)
        else :
            for decoder, up, enc_skip in zip(self.decoders_blur, self.ups_blur, encs[::-1]):
                x = up(x)
                x = x + enc_skip
                x = decoder(x)

            x = self.ending_blur(x)
...
class NAFNetLocal(Local_Base, NAFNet_multi):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        NAFNet_multi.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = NAFNet_multi(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
