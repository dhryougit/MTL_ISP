...
class UniformQuantize(InplaceFunction):
    ...
    def forward(cls, ctx, input, num_bits=8, min_value=None, max_value=None,
        ...
        if min_value is None or max_value is None:
            B = input.shape[0]
            y = input.reshape(B // num_chunks, -1)
...
# def quantize(x, num_bits=4, min_value=None, max_value=None, num_chunks=None, stochastic=False, inplace=False):
#     return UniformQuantize().apply(x, num_bits, min_value, max_value, num_chunks, stochastic, inplace)

# def quantize(x, num_bits=2, min_value=None, max_value=None, num_chunks=None, stochastic=False, inplace=False):
#     return UniformQuantize().apply(x, num_bits, min_value, max_value, num_chunks, stochastic, inplace)
