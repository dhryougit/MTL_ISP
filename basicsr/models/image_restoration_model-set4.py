...
class ImageRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        l_adv = 0.
        adv_preds = self.net_g(adv, self.filter_on)
        ...
        gt_filtered = torch.fft.fftn(self.gt, dim=(-1,-2))
        gt_filtered = torch.fft.fftshift(gt_filtered)
        gt_filtered = (gt_filtered*(mask.unsqueeze(1)))
        gt_filtered = torch.fft.ifftshift(gt_filtered)
        gt_filtered = torch.fft.ifftn(gt_filtered, dim=(-1,-2))
        gt_filtered = torch.clamp(gt_filtered.real, 0, 1)
        
   
        l_pix_filter = 0.
        l_pix_filter += self.cri_pix(preds, gt_filtered)
        ...
        loss_total = (1-self.opt['train']['filter_rate'])*loss_ori + self.opt['train']['filter_rate']*loss_filter
